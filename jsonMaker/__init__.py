...
from nooCrypto.mrkl import NooTree
...
class JsonPackets():
    ...
    @staticmethod
    def standartTransaction(cryptor:NooCrypto, reciever:str, token_count:int, token_type:str): # Стандартная транзакция перевода средств
    ...
    @staticmethod
    def standartTransactionWithoutSignature(sender:str, reciever:str, token_count:int, token_type:str, sign:str): # Стандартная транзакция перевода средств с указанием принудительной подписи
    ...
    @staticmethod
    def voteTransaction(cryptor:NooCrypto, reciever:str, vote_count:int): # Транзакция заявка на участие в следующем туре валидации
        temp = json.dumps({'TT': 'AT', 'SENDER': cryptor.getPublicKey(), 'RECEIVER': reciever, 'VOTES': vote_count, "TST": str(int(time.time()))}, separators=(',', ':'))
        sign = cryptor.signMessage(temp)
        jtemp = json.loads(temp)
        jtemp.update({'SIGNATURE':sign})
        return json.dumps(jtemp, separators=(',', ':'))
    ...
    @staticmethod
    def applicantTransaction(cryptor:NooCrypto, ip:str): # Транзакция заявка на участие в следующем туре валидации
        temp = json.dumps({'TT': 'AT', 'SENDER': cryptor.getPublicKey(), 'IPADDR': ip, "TST": str(int(time.time()))}, separators=(',', ':'))
        sign = cryptor.signMessage(temp)
        jtemp = json.loads(temp)
        jtemp.update({'SIGNATURE':sign})
        return json.dumps(jtemp, separators=(',', ':'))
    ...
    @staticmethod
    def createPrecommitBlock(cryptor:NooCrypto, ver:str, transactions:set, bheight:int, fee:float):
        jtransactions = []
        for tran in transactions:
            jtransactions.append(json.loads(tran))

        block = json.dumps({'TT': 'BL', 'SENDER': cryptor.getPublicKey(), 'VERSION': ver, 'TCOUNT': len(jtransactions), "BHEIGHT": bheight, 'TRANSACTIONS':jtransactions, 'FEE': fee}, separators=(',', ':'))
